...
let findLink = (task, callback) => {
    ...
    log.info(`Searching for ${task.style} ${task.keyword} in ${task.category}...`);
        ...
        if (err) {
            ...
            $('div#container').children().each(function(i, elem) {
                ...
                if (title.includes(task.keyword) && color.includes(task.style)) {
                    itemFound = true;
                    log.success(`${color} ${title} found!`);
                    task.link = link;
                    task.title = title;
                    return callback(task);
                } else if (title.includes(task.keyword) && color.includes(task.style) && $(this).children().text().indexOf('sold') != -1) {
                    itemFound = true;
                    log.warning(`${color} ${title} is sold out. Waiting for restock...`);
                    return setTimeout(findLink, config.refreshRate, task, callback);
                }
            });
            if (!itemFound) {
                log.warning(`Could not find any items matching ${task.style} ${task.keyword} searching again in ${config.refreshRate / 1000} seconds...`);
                return setTimeout(findLink, config.refreshRate, task, callback);
            }
        }
    })
...
let getCodes = (task, callback) => {
    log.info(`Getting size and style codes for ${task.color} ${task.title}...`);
    request({
        ...
        if (err) {
            ...
            if ($('b.sold-out').text() == 'sold out') {
                log.warning(`${style} ${title} is sold out. Waiting for restock...`);
                return setTimeout(getCodes, config.refreshRate, task, callback);
            }
            if (task.size == '') {
                item.sizeCode = $('#s').attr('value');
                log.success(`Found size code: ${task.sizeCode} and style code: ${task.styleCode} for ${task.size} ${task.style} ${task.title}!`)
                return callback(task);
            } else {
...
let addToCart = (task, callback) => {
    log.info(`Adding ${task.size} ${task.style} ${task.title} to cart...`);
    request({
        ...
        if (err) {
            ...
            log.success(`Added ${task.size} ${task.style} ${task.title} to cart!`);
            return callback(task);
        }
    });
}
...
let checkout = (task) => {
    request({
        ...
        if (err) {
            ...
            log.info(`Waiting for captcha response....`);
            if (config.captcha == 'open') {
                captcha.open(sitekey, 0, (capResp) => {
                    log.success(`Received captcha response ${capResp.substr(0, 10)}...`);
                    setTimeout(post, config.checkoutDelay, task, capResp);
                });
            }
            if (config.captcha == 'server') {
                captcha.server(sitekey, 0, moment(), (capResp) => {
                    log.info(`Received captcha response ${capResp.substr(0, 10)}...`);
                    setTimeout(post, config.checkoutDelay, task, capResp);
                });
            }

            function post(task, capResp) {
                log.info(`Attempting to checkout ${task.size} ${task.style} ${task.title}...`);
                request({
